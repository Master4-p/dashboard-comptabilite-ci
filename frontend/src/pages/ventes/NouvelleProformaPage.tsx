...
export default function NouvelleProformaPage() {
  ...
  // Load customers + proforma (if editing)
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        ...
        if (isEditing && id) {
          const proforma = await clientsApi.get(Number(id));
          if (proforma.type !== 'proforma') {
            addToast('error', 'Ce document n\'est pas une proforma.');
            navigate('/ventes/proformas');
            return;
          }
          setNumero(proforma.numero);
          setDateEmission(proforma.date_emission);
          setDateEcheance(proforma.date_relance || '');
          setDateValidite(proforma.date_validite || '');
          setNotes(proforma.notes);
          setAttachment(proforma.pj_filename);

          let customer = allCustomers.find(c => c.id === proforma.customer_id);
          if (!customer && proforma.customer_id) {
            try {
              customer = await customersApi.get(proforma.customer_id);
            } catch {
              customer = undefined;
            }
          }
          if (!customer) {
            customer = allCustomers.find(c => c.name === proforma.client);
          }
          setSelectedCustomer(customer || null);

          const lines = await lineItemsApi.list(proforma.id, 'proforma');
          ...
          const numbering = await numberingApi.get('PF', year);
          setNumero(numbering.formatted);
          await numberingApi.increment('PF', year);

          // Default validity date: 30 days from today
          const d = new Date();
          d.setDate(d.getDate() + 30);
          setDateValidite(d.toISOString().split('T')[0]);
        }
      } catch (e) {
        console.error(e);
  ...
  const handleSave = async (targetStatut: 'brouillon' | 'envoye') => {
    ...
    setSaving(true);
    try {
      let proforma: ClientItem;
      if (isEditing && id) {
        // Delete existing line items first, then update proforma
        const existingLines = await lineItemsApi.list(Number(id), 'proforma');
        await Promise.all(existingLines.map(l => lineItemsApi.remove(l.id)));
        proforma = await clientsApi.update(Number(id), {
        ...
        proforma = await clientsApi.create({
          type: 'proforma',
          numero,
          customer_id: selectedCustomer.id,
          client: selectedCustomer.name,
      ...
      addToast('success', isEditing ? 'Proforma mise à jour.' : 'Proforma créée avec succès.');
      navigate('/ventes/proformas');
    } catch (err) {
  ...
  return (
    <div className="max-w-[1100px] mx-auto" data-testid="nouvelle-proforma-page">
      <div className="flex items-center gap-2 mb-3">
        <button
          onClick={() => navigate('/ventes/proformas')}
          className="btn-icon hover:bg-slate-100 text-slate-500"
          title="Retour aux proformas"
          data-testid="proforma-back-btn"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <span className="text-xs text-[#94A3B8]">Retour à la liste des proformas</span>
      </div>
      ...
      <PageHeader
        breadcrumb={[
          ...
          { label: 'Proformas', to: '/ventes/proformas' },
          { label: isEditing ? 'Modifier' : 'Nouvelle' },
        ]}
        title={isEditing ? 'Modifier proforma' : 'Nouvelle proforma'}
        subtitle={isEditing ? `Proforma ${numero}` : 'Créez une proforma client.'}
      />

      {/* Document info section */}
      <div className="card mb-5" data-testid="proforma-info-section">
        <div className="form-section">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <div>
              <label className="form-label">N° proforma</label>
            ...
            <div>
              ...
              <div className="form-input bg-slate-50 flex items-center">
                <span className="badge-slate">Brouillon</span>
              </div>
            </div>
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  if (val && !isEditing) {
                    ...
                    setDateValidite(vd.toISOString().split('T')[0]);
                  }
                }}
                data-testid="proforma-date-emission"
              />
            </div>
            <div>
              ...
              <input
                ...
                data-testid="proforma-date-echeance"
              />
            </div>
            <div>
              ...
              <input
                ...
                data-testid="proforma-date-validite"
              />
            </div>
          </div>
        </div>
        ...
        <div className="form-section">
          ...
          <div className="relative">
            <input
              ...
              data-testid="proforma-customer-search"
            />
            {selectedCustomer && (
              <button
                onClick={() => {
      ...
      {/* Line items */}
      <div className="card mb-5" data-testid="proforma-lines-section">
        <div className="form-section">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <div className="form-section-title">Lignes de proforma</div>
              <div className="form-section-subtitle">Détaillez les prestations ou produits proposés</div>
            </div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {calculatedLines.map((line, index) => (
                  <tr key={index} data-testid={`proforma-line-${index}`}>
      ...
      {/* Sticky totals bar */}
      <div className="sticky bottom-0 bg-white border-t border-[#E2E8F0] shadow-lg z-10 py-3 px-4 md:px-6 -mx-4 md:-mx-6 lg:-mx-8 mt-4 rounded-b-xl" data-testid="proforma-totals-bar">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 max-w-[1100px] mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-x-6 gap-y-2 text-sm w-full lg:w-auto">
            ...
            <div>
              ...
              <div className="font-bold text-base font-mono text-[#111827]" data-testid="proforma-total">{formatFCFA(totals.grandTotal)}</div>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            <button
              ...
              data-testid="proforma-cancel-btn"
            >
              Annuler
            </button>
            <button
            ...
            <button
              ...
              className="btn btn-secondary btn-sm"
              disabled={saving}
              data-testid="proforma-save-draft-btn"
            >
            ...
            <button
              ...
              disabled={saving}
              data-testid="proforma-save-send-btn"
            >
