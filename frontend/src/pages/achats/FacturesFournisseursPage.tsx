...
export default function FacturesFournisseursPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      addToast('error', 'Erreur chargement des factures fournisseurs.');
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const filteredItems = useMemo(() => {
    let result = items.filter((i) => i.type === 'facture_fournisseur');
  ...
  const handleDelete = useCallback(async (id: number) => {
    ...
    try {
      ...
      fetchItems();
      addToast('success', 'Facture fournisseur supprimée.');
    } catch (e) {
